...
export const AuthProvider = ({ children }) => {
  ...
  const ensureUserWithAvatar = (userData) => {
    ...
    const profilePicture = userData.profile_picture || userData.avatarUrl || userData.profilePicUrl || localStorage.getItem(KEY_PROFILE_PIC) || getDefaultAvatar(userData);
    return {
      ...userData,
  ...
  const persistUser = (u) => {
    ...
    if (withAvatar.profile_picture) localStorage.setItem(KEY_PROFILE_PIC, withAvatar.profile_picture);
  };
  ...
  const updateProfilePicture = useCallback((newPictureUrl) => {
    if (user) {
      const updatedUser = ensureUserWithAvatar({ ...user, profile_picture: newPictureUrl, avatarUrl: newPictureUrl, profilePicUrl: newPictureUrl });
      setUser(updatedUser);
  ...
  useEffect(() => {
    const onStorage = (e) => {
      if (e.key === KEY_USER || e.key === KEY_PROFILE_PIC) {
        ...
        if (stored) setUser(ensureUserWithAvatar(JSON.parse(stored)));
        else {
          setUser(null);
          setIsAuthenticated(false);
        }
