...
/** Strip every non-digit (and non +) character from a phone string */
...
export default function AddPersonDialog({
  ...
  isEdit = false,
  personId = null,
  currentEventId,   // needed for updating new-people card in ServiceCheckIn
}) {
  const theme = useTheme();
  const isDark = theme.palette.mode === "dark";
  const { authFetch } = useContext(AuthContext);
  ...
  // Fetch full person when editing
  ...
  /**
   * Name / surname fields: capitalise every word on every keystroke.
   * Called from the renderTextField name/surname variant.
   */
  ...
  /**
   * Phone field: only allow digits (and leading +).
   */
  ...
  const renderTextField = (name, label, options = {}) => {
    ...
    // Choose the right onChange handler based on field
    ...
    return (
      <TextField
        ...
        // For number field, restrict keyboard on mobile
    ...
    try {
      ...
      if (isEdit && personId) {
        ...
        if (response.ok) {
          ...
          // ── FIX: also update this person inside realTimeData.new_people ──
          // ServiceCheckIn listens for onSave and will receive the normalizedData.
          // We pass a flag so the parent knows to patch the new_people list too.
