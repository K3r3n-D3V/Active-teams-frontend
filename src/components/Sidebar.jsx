...
export default function Sidebar({ mode, setMode }) {
  ...
  useEffect(() => {
    const checkUserAccess = async () => {
      ...
      console.log(
        ` Sidebar - User role: ${userRole}, Custom: ${isCustomRole}, Supreme: ${isSupremeAdmin}`,
      );
      ...
      const filteredItems = allMenuItems.filter((item) => {
        if (isCustomRole) {
          ...
          if (item.level > userLevel) {
            console.log(
              ` ${item.label}: Custom role ${userRole} level ${userLevel} < required ${item.level}`,
            );
            return false;
          }
          ...
          console.log(
            `${item.label}: Custom role ${userRole} granted access (level ${userLevel})`,
          );
          return true;
        } else {
          ...
          if (!itemRolesLower.includes(userRole)) {
            console.log(
              ` ${item.label}: System role ${userRole} not in ${item.roles}`,
            );
            return false;
          }
