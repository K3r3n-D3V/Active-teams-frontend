...
const StatsDashboard = () => {
    ...
    try {
      ...
      try {
        ...
        if (tasksResponse.ok) {
          ...
          const rawTasks = tasksData.tasks || tasksData.results || [];

          const filteredTasks = rawTasks.filter((task) => {
            const rawDate = task.date || task.followup_date;
  ...
  const handleDateSelect = (date) => {
    ...
    if (eventsOnDate.length > 0) {
      setSnackbar({
        open: true,
        message: `${eventsOnDate.length} event(s) on ${formatDisplayDate(
          date
        )}`,
        severity: "info",
      });
  ...
  const EnhancedCalendar = () => {
    ...
    return (
      <Box>
        ...
        >
          <Typography
            variant={getResponsiveValue("body1", "h6", "h6", "h6", "h6")}
            fontWeight="bold"
          >
  ...
  const StatCardSkeleton = () => (
    ...
    >
      ...
      >
        <Skeleton
          variant="circular"
          width={getResponsiveValue(20, 24, 28, 32, 32)}
          height={getResponsiveValue(20, 24, 28, 32, 32)}
        />
        <Skeleton
          variant="text"
          width="40%"
      ...
      <Skeleton
        variant="text"
        width="70%"
        height={getResponsiveValue(16, 18, 20, 22, 24)}
        ...
        <Typography variant="caption" color="textSecondary" noWrap>
          {type === "cells" ? (
            <>
              ...
              {item.eventLeaderName || "No leader"}
            </>
          ) : (
            <>
    ...
    >
      <DialogTitle sx={{ pb: 1, fontWeight: 600 }}>
        <Box display="flex" alignItems="center" justifyContent="space-between">
          <Typography
            variant={getResponsiveValue("h6", "h6", "h5", "h5", "h5")}
          >
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={(e) =>
                  setNewEventData({
                    ...newEventData,
                    eventLeaderName: e.target.value,
                  })
                }
                fullWidth
                size={getResponsiveValue(
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={(e) =>
                  setNewEventData({
                    ...newEventData,
                    eventLeaderEmail: e.target.value,
                  })
                }
                fullWidth
                size={getResponsiveValue(
  ...
  return (
    <Box
      p={containerPadding}
      sx={{ maxWidth: "1400px", margin: "0 auto", mt: 8, minHeight: "100vh" }}
    >
      <CssBaseline />
      ...
      <Box sx={{ mb: cardSpacing }}>
        ...
        {(refreshing || stats.loading) && <LinearProgress sx={{ mt: 1 }} />}
      </Box>
      ...
      <Box sx={{ minHeight: "400px" }}>
        {activeTab === 0 && (
          <Paper
            variant="outlined"
            sx={{ boxShadow: 3, p: isSmDown ? 1 : 2, height: "100%" }}
          >
            <Box
              display="flex"
              alignItems="center"
