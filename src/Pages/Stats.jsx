...
const isOverdue = useCallback((cell) => {
  ...
  /**
   * Returns the effective status for a task.
   * An open task whose creation date is before today is considered "overdue".
   */
  const getTaskStatus = useCallback((task) => {
    ...
    // If the task is already complete, honour that status
    if (completedStatuses.includes(rawStatus)) return task.status;

    // Determine the reference date: prefer followup_date, fall back to created_at
    const refDateStr = task.followup_date || task.created_at;
    if (!refDateStr) return task.status || "pending";

    const refDate = new Date(refDateStr);
    if (isNaN(refDate.getTime())) return task.status || "pending";
      ...
      if (activeTab === 0) {
        ...
        dataToExport = filteredTasks.map((task) => ({
          ...
          "Due Date": task.followup_date
            ...
            : "",
          Status: getTaskStatus(task) || "pending",
                              ...
                              <Stack spacing={1}>
                                {tasks.map((task) => (
                                  ...
                                  >
                                    ...
                                    <Chip
                                      label={getTaskStatus(task) || "Pending"}
                                      size="small"
                                      color={
                                        ["completed", "done", "complete", "closed"].includes(
                                          ...
                                          ? "success"
                                          : getTaskStatus(task)?.toLowerCase?.() === "overdue"
                                            ? "error"
                                            : "warning"
                                      }
