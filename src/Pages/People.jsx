...
import { 
  ...
  TextField, InputAdornment, Button, Snackbar, Alert, AppBar, Toolbar, Slide,
  Popover  // <-- add this
} from '@mui/material';
import {
              ...
              <Paper key={stage.id} sx={{ flex: `0 0 ${stageWidth}`, minWidth: 220, borderRadius: 2, overflow: 'hidden', mb: isSmall || isMedium ? 2 : 0, backgroundColor: theme.palette.mode === 'dark' ? theme.palette.grey[800] : theme.palette.common.white }}>
                <Box sx={{ p: 1.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: theme.palette.mode === 'dark' ? theme.palette.grey[900] : theme.palette.grey[100], color: theme.palette.mode === 'dark' ? theme.palette.grey[100] : theme.palette.grey[900] }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Badge badgeContent={stagePeople.length} sx={{ '& .MuiBadge-badge': { backgroundColor: theme.palette.mode === 'dark' ? '#fff' : '#000', color: theme.palette.mode === 'dark' ? '#000' : '#fff', fontSize: 10, transform: 'translateY(1px)', minWidth: 16, height: 16 } }} >
                ...
                <Droppable droppableId={stage.id}>
                  {(provided) => (
                    <Box ref={provided.innerRef} {...provided.droppableProps} sx={{ p: 1, minHeight: 140, maxHeight: '400px', overflowY: 'auto', overflowX: 'hidden', backgroundColor: theme.palette.mode === 'dark' ? theme.palette.grey[700] : theme.palette.grey[50] }}>
                      <List
                        height={400}
                        itemCount={stagePeople.length}
                        itemSize={90} // adjust based on Card height
                        width="100%"
                      >
                        {({ index, style }) => {
                          const person = stagePeople[index];
                          return (
...
export const PeopleSection = () => {
  ...
  try {
    ...
    const data = Array.isArray(res.data?.results)
      ? res.data.results.map(p => ({
          ...
          stage: "Win", // <-- default everyone to Win
          lastUpdated: p.UpdatedAt || null,
          leaders: [p["Leader @12"] || "", p["Leader @144"] || "", p["Leader @ 1728"] || ""]
      })) : [];
    setPeople(data);
  } catch (err) {
  ...
  const filteredPeople = useMemo(() =>
    people.filter(p => {
      ...
      const leaderStr = [(p.cellLeader || ''), ...(p.leaders || [])].join(' ');
      return (p.name?.toLowerCase().includes(search)) ||
             (p.surname?.toLowerCase().includes(search)) ||
             leaderStr.toLowerCase().includes(search);
    }),
    [people, searchTerm]
  );
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', mt: 8, px: 2, pb: 4 }}>
      ...
      <Snackbar
        ...
        sx={{ mt: 2, ml: { xs: '5%', sm: '10%', md: '15%' }, width: { xs: '90%', sm: 'auto' } }}
      >
        <Alert onClose={() => setSnackbar({ ...snackbar, open: false })} severity={snackbar.severity} variant="filled" sx={{ width: '100%', borderRadius: 2, boxShadow: 3, px: 3, py: 1.5, textAlign: 'center', fontWeight: 500 }}>
          {snackbar.message}
        </Alert>
      </Snackbar>
    </Box>
