...
const safeStr = (v) => {
  ...
  if (typeof v === 'object') { try { return String(v); } catch { return ''; } }
  return '';
};
...
function getLeadersByLevel(person) {
  if (Array.isArray(person?.leaders) && person.leaders.length > 0) {
    ...
    for (const l of person.leaders) {
      if (l?.level != null && l?.name) map[`leader${l.level}`] = safeStr(l.name);
    }
...
const StatCardSkeleton = () => (
  <Card sx={{ width: '100%', height: 200, display: 'flex', flexDirection: 'column',
    justifyContent: 'center', alignItems: 'center', boxShadow: 2 }}>
...
const BoardSkeleton = ({ cardCount = 4 }) => {
  ...
  return (
    <Box sx={{ display: 'flex', flexWrap: isSmall ? 'wrap' : 'nowrap', gap: 3, justifyContent: 'center', py: 1 }}>
      {stages.map(stage => (
        <Paper key={stage.id} sx={{
          flex: isSmall ? '1 1 100%' : '0 0 250px', minWidth: 220,
          ...
          {/* Card skeletons */}
          <Box sx={{ p: 1 }}>
            {[...Array(cardCount)].map((_, i) => <PersonCardSkeleton key={i} />)}
          </Box>
        </Paper>
      ))}
    </Box>
          ...
          return (
            <Paper key={stage.id} sx={{
              ...
              <Box sx={{ p: 1.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                ...
                <Badge badgeContent={stagePeople.length} sx={{
                  '& .MuiBadge-badge': {
              ...
              <Droppable droppableId={stage.id}>
                {(provided) => (
                  <Box ref={provided.innerRef} {...provided.droppableProps}
                    sx={{ p: 1, minHeight: 140, maxHeight: '400px', overflowY: 'auto' }}>
                    {stagePeople.length > 0
                      ...
                      : (
                        ...
                        loading
                          ? [...Array(2)].map((_, i) => <PersonCardSkeleton key={i} />)
                          : <Typography variant="body2" sx={{ textAlign: 'center', py: 2, color: 'text.disabled' }}>No people</Typography>
                      )
                    }
...
export const PeopleSection = () => {
  ...
  const [allPeople,       setAllPeople]       = useState(() => hasCachedData ? window.globalPeopleCache : []);
  ...
  const [cacheInfo,       setCacheInfo]       = useState({ source: hasCachedData ? 'local-cache' : '', org: '' });
  ...
  const searchPeople = useCallback((list, q, field = 'name') => {
    ...
    if (field === 'stage') return list.filter(p => safeStr(p.Stage).toLowerCase().includes(lq));
            ...
            <Grid item xs={6} sm={4} md={2.4} key={stat.id}>
              {loading ? (
                ...
                <Card sx={{
                  ...
                  <CardContent sx={{ textAlign: 'center', p: 2, width: '100%', display: 'flex',
                    flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
                    <Avatar sx={{ bgcolor: stat.color, width: 56, height: 56, mb: 1, boxShadow: 2 }}>
                      {stat.icon}
                    </Avatar>
        ...
        {loading ? (
          ...
          <>
            <TextField
              ...
              sx={{ flex: 1 }}
            />
            <TextField size="small" select value={searchField}
              onChange={e => setSearchField(e.target.value)} sx={{ minWidth: 140 }}>
