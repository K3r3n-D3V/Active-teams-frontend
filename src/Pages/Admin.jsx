...
  sx={{
    mb: cardSpacing,
    alignItems: 'stretch' // This ensures all items in a row have equal height
  }}
>
  {[
...
      sm={4}
      md={2}
      sx={{ display: 'flex' }} // Forces the card to expand to the grid's height
    >
      <Card sx={{ 
        ...cardStyles, 
        width: '100%', 
...
            sx={{ 
              mt: 1,
              minHeight: '2.4em', // Reserves space for 2 lines so heights match
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center'
            }}
...
                        </Avatar>
                        <Typography variant="h6">{getRoleDisplay(role.name)}</Typography>
                      </Stack>
        ...
        {activeTab === 2 && (
          <Box sx={{ p: 3 }}>
            <List>
              ...
              {activityLog.length === 0 && <Typography>No activity recorded yet.</Typography>}
            </List>
          </Box>
        )}
      </Paper>
      ...
      <Dialog open={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)}>
        ...
        <DialogActions>
          ...
          <Button color="error" variant="contained" onClick={handleDeleteUser} disabled={deletingUser}>
            {deletingUser ? <CircularProgress size={24} /> : 'Delete'}
          </Button>
        </DialogActions>
      </Dialog>

      <Dialog open={showRoleModal} onClose={() => setShowRoleModal(false)} fullWidth maxWidth="xs">
        <DialogTitle>Update Role: {selectedUser?.name}</DialogTitle>
        <DialogContent>
          <List>
            {roles.map((role) => (
              <ListItem 
              ...
              >
                ...
                <ListItemText primary={getRoleDisplay(role.name)} secondary={role.description} />
              </ListItem>
            ))}
          </List>
        </DialogContent>
