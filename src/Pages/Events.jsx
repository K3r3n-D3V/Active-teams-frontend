  ...
  const generateDynamicColumns = (events) => {
    ...
    const filteredFields = Object.keys(sampleEvent).filter((key) => {
      ...
      const excludedFields = [
        ...
        'leader12', 'leader@12', 'leader at 12', 'original_event_id'
      ];
      ...
      const shouldExclude = exactMatch || caseInsensitiveMatch || containsOverdue ||
        containsDisplayDate || containsOriginated || containsLeader12;
  ...
  const MobileEventCard = ({ 
    ...
    isAdmin,
    isLeaderAt12,
    currentUserLeaderAt1 // ✅ ADD THIS PROP
  }) => {
    if (!theme) {
  ...
  const fetchEvents = useCallback(async (filters = {}, forceRefresh = false) => {
    console.log("🔍 fetchEvents - Leader at 12 DEBUG:", {
      isLeaderAt12,
      viewFilter,
    ...
    try {
      ...
      // ✅ FIXED: Build proper params for leader at 12
      const params = {
      ...
      // ✅ CRITICAL FIX: Enhanced leader at 12 parameters
      if (isLeaderAt12) {
        params.leader_at_12_view = true;
        params.include_subordinate_cells = true;
        
        // Add current user's leader at 1 identifier if available
        if (currentUserLeaderAt1) {
          params.leader_at_1_identifier = currentUserLeaderAt1;
        }
        
        if (viewFilter === 'personal') {
          // Show only their own cells
          params.show_personal_cells = true;
          params.personal = true;
        } else {
          // Show all cells where they are Leader at 12 (including disciples)
          params.show_all_authorized = true;
          params.include_subordinate_cells = true;
        }
        
        console.log("🎯 ENHANCED LEADER AT 12 PARAMS:", params);
      }
      ...
      if (eventType === 'all' || eventType === 'CELLS' || !eventType) {
        endpoint = `${BACKEND_URL}/events/cells`;
        console.log("Using CELLS endpoint with params:", params);
      } else {
        endpoint = `${BACKEND_URL}/events/other`;
        console.log("Using OTHER EVENTS endpoint for:", eventType);
      }

      Object.keys(params).forEach(key => (params[key] === undefined || params[key] === null) && delete params[key]);

      console.log('Final API call:', { endpoint, params });
...
// ✅ CRITICAL DEBUG: Enhanced logging that WILL show results
console.log('🔍 ========== BACKEND RESPONSE DEBUG ==========');
console.log('🔍 Endpoint:', endpoint);
console.log('🔍 Params sent:', JSON.stringify(params, null, 2));
console.log('🔍 Response data structure:', Object.keys(responseData));
console.log('🔍 Total events in response:', responseData.total_events);
console.log('🔍 Events array length:', newEvents.length);

if (newEvents.length > 0) {
  console.log('🔍 First 3 events sample:', newEvents.slice(0, 3).map(e => ({
    name: e.eventName,
...
// ✅ SPECIFIC DEBUG FOR EVENT TYPE FILTERING
if (params.event_type && params.event_type !== 'all' && params.event_type !== 'CELLS') {
  console.log('🎯 ========== EVENT TYPE FILTERING DEBUG ==========');
  console.log('🎯 Event type requested:', params.event_type);
  
  const eventsWithMatchingType = newEvents.filter(e => 
    e.eventType === params.event_type || e.eventTypeName === params.event_type
  );
  
  console.log('🎯 Events with exact match:', eventsWithMatchingType.length);
  
  if (eventsWithMatchingType.length > 0) {
    console.log('🎯 Matching events:', eventsWithMatchingType.map(e => ({
      name: e.eventName,
      eventType: e.eventType,
      eventTypeName: e.eventTypeName
    })));
  } else {
    console.log('❌ NO EVENTS MATCH THE REQUESTED EVENT TYPE');
    console.log('🎯 All event types in response:', [...new Set(newEvents.map(e => e.eventType || e.eventTypeName).filter(Boolean))]);
  }
}
...
const refreshAfterEventTypeUpdate = useCallback(async (oldName, newName) => {
  try {
    ...
    // ✅ Force events refresh with new type
    setTimeout(() => {
      const refreshParams = {
        page: 1,
        limit: rowsPerPage,
...
const debugCurrentSituation = async () => {
  ...
  try {
    ...
    }, [selectedEvent, currentUser, BACKEND_URL, fetchEvents]);
    ...
    }, [clearCache, currentPage, rowsPerPage, selectedStatus, searchQuery, selectedEventTypeFilter, fetchEvents, DEFAULT_API_START_DATE]);
    ...
    // Add this useEffect to initialize the event type filter properly
    useEffect(() => {
...
const handleSaveEventType = useCallback(async (eventTypeData, eventTypeId = null) => {
  try {
    ...
    // ✅ CRITICAL FIX: Close modal first to avoid state conflicts
    setEventTypesModalOpen(false);
    setEditingEventType(null);

    // ✅ IMMEDIATELY update the filter if we were viewing the old event type
    if (selectedEventTypeFilter === oldName) {
      console.log(`🔄 Immediately updating filter from '${oldName}' to '${newName}'`);
      setSelectedEventTypeFilter(newName);
    }

    // ✅ Force refresh event types
    await fetchEventTypes();

    // ✅ Clear all caches aggressively
    clearCache();
    const cacheKeys = Object.keys(localStorage).filter(key => 
      key.includes('cache') || key.includes('events_cache')
    );
    cacheKeys.forEach(key => localStorage.removeItem(key));
    ...
    // ✅ FORCE REFRESH with the correct event type
    const refreshParams = {
      page: 1,
      limit: rowsPerPage,
      start_date: DEFAULT_API_START_DATE,
      _t: Date.now(),
      // ✅ ALWAYS use the new event type name for the fetch
      event_type: newName
    };
    ...
    console.log("🔍 Force refreshing events with:", refreshParams);
    ...
    setTimeout(async () => {
      console.log("🔄 Double-checking events are loaded...");
      await fetchEvents(refreshParams, true);
    }, 1000);

    setSnackbar({
      open: true,
      message: `Event type ${eventTypeId ? 'updated' : 'created'} successfully! Events should now be visible.`,
      severity: "success",
    });
...
}, [BACKEND_URL, editingEventType, fetchEventTypes, fetchEvents, selectedEventTypeFilter, rowsPerPage, selectedStatus, searchQuery, DEFAULT_API_START_DATE, clearCache]);
  ...
  useEffect(() => {
    ...
    // ✅ ENHANCED: Better Leader at 12 filtering
    if (selectedEventTypeFilter === 'all' || selectedEventTypeFilter === 'CELLS') {
      if (isLeaderAt12) {
        // ✅ CRITICAL: Always include these parameters for leader at 12
        ...
        if (viewFilter === 'personal') {
          ...
          console.log("🔍 Leader at 12: Showing PERSONAL cells only");
        } else {
          // Show all cells where they are Leader at 12
          fetchParams.show_all_authorized = true;
          fetchParams.include_subordinate_cells = true;
          console.log("🔍 Leader at 12: Showing ALL authorized cells (own + subordinates)");
        }
        
        console.log("🎯 FINAL Leader at 12 params:", fetchParams);
      } else if (isAdmin) {
        // Admin: Use personal filter
        if (viewFilter === 'personal') {
          fetchParams.personal = true;
        }
      } else if (isRegistrant || isRegularUser) {
        // Registrants and regular users ALWAYS see personal only
        fetchParams.personal = true;
      }
    }
  ...
  const testLeaderAt12Backend = async () => {
    try {
      const token = localStorage.getItem("token");
      const testParams = {
      ...
      if (currentUserLeaderAt1) {
        testParams.leader_at_1_identifier = currentUserLeaderAt1;
      }

      console.log("🧪 TESTING Backend with params:", testParams);

      const response = await axios.get(`${BACKEND_URL}/events/cells`, {
        headers: { Authorization: `Bearer ${token}` },
        params: testParams
      });
      ...
      console.log("🧪 BACKEND TEST RESPONSE:", {
        ...
        events: response.data.events?.map(e => ({
          name: e.eventName,
          leader: e.eventLeaderName,
          leader12: e.leader12 || e['Leader at 12'] || e.leaderAt12,
          leader1: e.leader1 || e.Leader1,
          isSashaSubordinate: (e.leader12 || e['Leader at 12'] || e.leaderAt12) === "Sasha-Lee Enslin"
        }))
      });
  ...
  const ViewFilterButtons = () => {
    ...
    (isAdmin || (isLeaderAt12 && !isCheckingLeaderStatus)) && 
    ...
    const handleViewFilterChange = (newViewFilter) => {
      ...
      // ✅ ENHANCED: Better Leader at 12 logic
      if (isLeaderAt12) {
        fetchParams.leader_at_12_view = true;
        fetchParams.include_subordinate_cells = true;
        ...
        console.log(`🎯 Leader at 12 view changed to ${newViewFilter}:`, fetchParams);
      } else if (isAdmin) {
        fetchParams.personal = newViewFilter === 'personal';
      }
      ...
      <div style={styles.viewFilterContainer}>
        ...
        <label style={styles.viewFilterRadio}>
          ...
          <span style={styles.viewFilterText}>
            {isLeaderAt12 ? "VIEWL ALL" : "PERSONAL"}
          </span>
        </label>
        <label style={styles.viewFilterRadio}>
          <input
  ...
  const EventTypeSelector = ({
    ...
    // ✅ ADD THIS: Check if current user can edit event types
    const canEditEventTypes = isAdmin; // Only admins can edit event types
...
const checkAllEvents = async () => {
  try {
    ...
    // Check which events should be under "Conferencetestt"
    const conferencetesttEvents = response.data.events?.filter(e => 
      e.eventType === "Conferencetestt" || e.eventTypeName === "Conferencetestt"
    );
    
    console.log("🎯 EVENTS THAT SHOULD BE UNDER 'Conferencetestt':", {
      count: conferencetesttEvents?.length,
      events: conferencetesttEvents?.map(e => ({
        name: e.eventName,
        eventType: e.eventType,
        eventTypeName: e.eventTypeName
      }))
      ...
      <div style={mobileEventTypeStyles.container}>
        ...
        <div style={mobileEventTypeStyles.typesGrid}>
          {allTypes.map((type) => {
            ...
            // ✅ FIXED: Only show menu for admin users AND non-"all" types
              ...
              <Box sx={{ flex: 1, overflowX: 'auto' }}>
                {loading ? (
                  ...
                  <DataGrid
                    ...
                    columns={[
                      ...generateDynamicColumns(paginatedEvents),
                      {
                        field: 'actions',
                        headerName: 'Actions',
                        sortable: false,
