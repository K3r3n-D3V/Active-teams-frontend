...
export default function Profile() {
  ...
  const [form, setForm] = useState({
    name: "", surname: "", dob: "", email: "", address: "",
    phone: "", invitedBy: "", gender: "", organization: "",
  ...
  const getUserRole = useCallback(() => {
    ...
    const parts = String(loggedInUserRole).trim()
      .split(/[\/,\s|]+/)
      .map(r => r.trim())
      .filter(Boolean)
  ...
  const updateFormWithProfile = useCallback((profile) => {
    ...
    // Leaders — stored as flat columns in your Users table
    const leaderNames = {
      leaderAt1:   profile?.leader12   ? `${profile.leader12}`   : (profile?.invited_by || ""),
      leaderAt12:  profile?.leader144  ? `${profile.leader144}`  : "",
      leaderAt144: profile?.leader1728 ? `${profile.leader1728}` : "",
    };

    // If the backend returned resolved leader objects (legacy path), use them
    if (profile?.leaders?.leaderAt1) {
      leaderNames.leaderAt1 = `${profile.leaders.leaderAt1.name || ""} ${profile.leaders.leaderAt1.surname || ""}`.trim();
    }
    if (profile?.leaders?.leaderAt12) {
      leaderNames.leaderAt12 = `${profile.leaders.leaderAt12.name || ""} ${profile.leaders.leaderAt12.surname || ""}`.trim();
    }
    if (profile?.leaders?.leaderAt144) {
      leaderNames.leaderAt144 = `${profile.leaders.leaderAt144.name || ""} ${profile.leaders.leaderAt144.surname || ""}`.trim();
    }
    ...
    const loadProfile = async () => {
      try {
        ...
        const email =
          authUser?.email ||
          (() => {
            try { return JSON.parse(localStorage.getItem("userProfile"))?.email; } catch { return null; }
          })();
  ...
  const validate = () => {
    ...
    if (form.newPassword || form.confirmPassword || form.currentPassword) {
      ...
      if (form.newPassword && form.newPassword.length < 8) n.newPassword = "Must be at least 8 characters";
  ...
  const onCropSave = async () => {
    try {
      ...
      if (!userId) {
        ...
        setSnackbar({ open: true, message: "Picture saved locally (no user ID found)", severity: "warning" });
        return;
      }
      ...
      try {
        ...
        const updatedProfile = { ...userProfile, profile_picture: publicUrl, avatarUrl: publicUrl, profilePicUrl: publicUrl };
  ...
  // ── Misc helpers ───────────────────────────────────────────────────────────
  const getInitials = () => (form.name || userProfile?.name || "").charAt(0).toUpperCase();
  ...
  if (loadingProfile) {
    return (
      <Box sx={{ minHeight: "100vh", bgcolor: isDark ? "#0a0a0a" : "#f8f9fa", pb: 4 }}>
        <Box sx={{ minHeight: "30vh", background: isDark ? `linear-gradient(135deg, ${currentCarouselItem.color}15 0%, ${currentCarouselItem.color}25 100%)` : `linear-gradient(135deg, ${currentCarouselItem.color}10 0%, ${currentCarouselItem.color}20 100%)`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", pt: 6, pb: 12 }}>
        ...
        <Box sx={{ display: "flex", justifyContent: "center", mt: -10, mb: 5 }}>
          <Skeleton variant="circular" width={150} height={150} sx={{ bgcolor: isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)" }} />
        </Box>
        <Container maxWidth="md" sx={{ px: { xs: 2, sm: 3 } }}>
          <Card sx={{ bgcolor: isDark ? "#111111" : "#ffffff", borderRadius: 3, border: `1px solid ${isDark ? "#222222" : "#e0e0e0"}` }}>
            <CardContent sx={{ p: { xs: 3, sm: 4 } }}>
              <Grid container spacing={3}>
                {[...Array(9)].map((_, i) => (
                  <Grid size={{ xs: 12, sm: 6 }} key={i}>
                    <Skeleton variant="text"        width="40%" height={20} sx={{ mb: 1, bgcolor: isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)" }} />
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: isDark ? "#0a0a0a" : "#f8f9fa", pb: 4 }}>
          ...
          <Box sx={{ position: "relative", display: "inline-block" }}>
            <Avatar
              sx={{
                ...
                color: currentCarouselItem.color, fontSize: "2.5rem", fontWeight: 700,
            ...
            <IconButton
              sx={{
                ...
                bgcolor: currentCarouselItem.color, color: "white", width: 36, height: 36,
            ...
            <Box component="form" onSubmit={handleSubmit}>
              <Grid container spacing={3}>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  ...
                  <TextField value={form.dob} onChange={handleChange("dob")} fullWidth type="date" disabled={!canEditProfile} error={!!errors.dob} helperText={errors.dob} slotProps={{ inputLabel: { shrink: true } }} sx={commonFieldSx} />
                </Grid>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  ...
                  <TextField select value={form.gender} onChange={handleChange("gender")} fullWidth disabled={!canEditProfile} sx={commonFieldSx}>
                    {[{ value: "", label: "Select Gender" }, { value: "Male", label: "Male" }, { value: "Female", label: "Female" }].map((o) => (
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  ...
                  <TextField value={form.phone} onChange={handleChange("phone")} fullWidth error={!!errors.phone} helperText={errors.phone} slotProps={{ htmlInput: { inputMode: "numeric", pattern: "[0-9]*" } }} sx={commonFieldSx} />
                </Grid>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  ...
                  <TextField value={organization || form.organization} onChange={handleChange("organization")} fullWidth disabled={!canEditProfile} placeholder="Your church or organization" sx={commonFieldSx} />
                </Grid>
              ...
              <Box sx={{ mt: 4 }}>
                ...
                >
                  ...
                  >
                    ...
                    <Typography variant="h6" sx={{ fontWeight: 600, color: isDark ? "#ffffff" : "#000000" }}>Advanced Settings</Typography>
                    <Typography variant="caption" sx={{ color: isDark ? "#999999" : "#666666", ml: 1 }}>(Change Password)</Typography>
                  </AccordionSummary>
                  <AccordionDetails>
                    <Grid container spacing={3}>
                      <Grid size={{ xs: 12 }}>
              ...
              <Box sx={{ mt: 4, display: "flex", gap: 2, justifyContent: "center", flexWrap: "wrap" }}>
                <Button
                  type="submit" variant="contained" startIcon={<Save />} disabled={!hasChanges}
                  sx={{
                ...
                {hasChanges && (
                  <Button
                    variant="outlined" onClick={handleCancel} startIcon={<Cancel />}
      ...
      {croppingOpen && (
        <Box
          sx={{ position: "fixed", inset: 0, bgcolor: "rgba(0,0,0,0.9)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1300, p: 2 }}
          onClick={() => setCroppingOpen(false)}
        >
          <Paper
            sx={{ position: "relative", width: "90vw", maxWidth: 500, bgcolor: isDark ? "#111111" : "#ffffff", borderRadius: 3, p: 3, border: `1px solid ${isDark ? "#333333" : "#e0e0e0"}` }}
          ...
          >
            ...
            <Box sx={{ mt: 2 }}>
              ...
              <Slider value={zoom} min={1} max={3} step={0.1} onChange={(_, v) => setZoom(v)}
                sx={{ color: currentCarouselItem.color, "& .MuiSlider-thumb": { bgcolor: currentCarouselItem.color }, "& .MuiSlider-rail": { bgcolor: isDark ? "#333333" : "#cccccc" } }}
              />
            </Box>
            <Box sx={{ mt: 3, display: "flex", gap: 2, justifyContent: "center" }}>
              <Button variant="outlined" onClick={() => setCroppingOpen(false)}
                sx={{ borderColor: isDark ? "#666" : "#ccc", color: isDark ? "#ccc" : "#666", borderRadius: 2, px: 3, py: 1, fontWeight: 600, textTransform: "none" }}>
                Cancel
              </Button>
              <Button variant="contained" onClick={onCropSave}
                sx={{ bgcolor: currentCarouselItem.color, "&:hover": { bgcolor: currentCarouselItem.color, opacity: 0.9 }, borderRadius: 2, px: 3, py: 1, fontWeight: 600, textTransform: "none" }}>
                Save Picture
              </Button>
            </Box>
          </Paper>
